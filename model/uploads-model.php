...
function getThumbsById($id) {
   $db = phpmotorsConnect();
   $sql = "SELECT i.imgPath, inv.invMake, inv.invModel FROM images i, inventory inv WHERE i.invId = inv.invId AND imgName LIKE '%-tn%' AND i.invId = :invId";
   $stmt = $db->prepare($sql);
   $stmt->bindValue(':invId', $id, PDO::PARAM_STR);
   $stmt->execute();
   $thumbsArray = $stmt->fetchAll();
   $stmt->closeCursor();
   return $thumbsArray;
}
