...
-- PHP Version: 7.3.7
-- ----------------------------------------------------------------------------
-- ---------- The following queries were provided in phpmotors.sql ------------
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- ----------        End provided queries from phpmotors.sql       ------------
-- ----------------------------------------------------------------------------
...
-- - NOTE: The following clients statements were originally commented out in
-- - phpmotors.sql. I have modified them to work according to the requirements
-- - in enhancement2 assignment 
DROP TABLE IF EXISTS `clients`;
CREATE TABLE `clients` (
  -- - modified 
...
-- - Updating clientLevel
UPDATE clients 
   SET clientLevel = 3 
      WHERE clientEmail 
         = "tony@starkent.com";

-- - Updating "GM Hummer" record to read "spacious interior" 
UPDATE inventory
   SET invDescription = replace(invDescription, 'small interior', 'spacious interior')
      WHERE  invModel = 'Hummer';

-- - inner join to retrieve SUV records from inventory table 
 SELECT i.invModel, c.classificationName 
   FROM inventory i, carclassification c 
   WHERE  i.classificationId = c.classificationId 
      AND c.classificationName = "SUV";

-- - deleting Jeep from inventory
DELETE FROM inventory 
   WHERE invMake = "Jeep";

-- - Updating all records in the inventory table to add "/phpmotors" to the 
-- - beginning of the file path in the invImage and invThumbnail columns
UPDATE inventory 
